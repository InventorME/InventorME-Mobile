...
const CreateAccountScreen = (props) => {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      createAlert(
        "NOTE",
        "Please confirm account by clicking on the link sent to your email, then sign in"
      );
  ...
  const phoneOnChange = (text) => {
    ...
    setPhone(cleaned);
    if (cleaned.length < 6) setPhoneFormat("+1 (" + cleaned.substring(2, 5));
  ...
  const validateUser = () => {
    if (name === "") {
      ...
      createAlert(
        "Create Account Error",
        "Email must be in the correct format 'Example@Example.Example'"
      );
    } else {
      submit();
    }
